...
//				   kiotUrl, HttpMethod.POST, entity, String.class).getBody();
//			   }
	   }
	   ...
	   private void initiateJob() throws ParseException {
		   final long HOUR = 3600 * 1000; // in milli-seconds.
			...
			//Date dnew=new Date(new Date().getTime() - 15*ONE_MINUTE_IN_MILLIS);
			Date dnew=new Date(new Date().getTime() +5*HOUR+HALFHOUR-15*ONE_MINUTE_IN_MILLIS);
		   ...
		   for (User user : users ) {
			   ...
			   for (UserDevices userdevice: userDevices) {
				   JSONObject kiotRequest=  createRequest.createKiotRequest(userdevice.getDeviceId(),dnew);
				 ArrayList<JSONObject> response = connectorUtil.connectToKiot(kiotUrl, kiotRequest, user.getBearerToken());
